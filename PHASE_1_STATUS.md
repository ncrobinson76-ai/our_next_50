...
| 11 | AI scenario suite and security/privacy acceptance tests pass before real-data launch | The 16-scenario suite (`packages/eval-harness/scenarios/`) runs successfully — verified as part of this package (2 scenarios spot-checked directly; the full suite was run and passed at Package 0's original construction). All 92 automated tests across `packages/api/test/` pass (see Part B below), including every cross-account isolation test. | The AI scenario suite's **manual rubric scoring completion is not independently verifiable from this repository** — see item 5's note. "Pass" here means: the suite runs without error and produces well-formed, scoreable output; it does not mean a human has certified passing scores are on file. |
...
3. **The AI scenario suite's manual scoring completion isn't verifiable
   from the repository.** The mechanism to score it exists and runs; a
   recorded, passing scorecard does not appear to exist in this repo.
