...
// Originally authored in packages/eval-harness (Package 0) against fictional
// scenarios; moved here unmodified in Package 9 so packages/eval-harness and
// packages/api can both build EvidencePackets (from fictional scenarios and
// real user data respectively) and hand them to the same synthesis engine.
// Do not add fields here to accommodate one caller's convenience — anything
// caller-specific belongs in that caller's own mapping layer instead.
